...
class PartTest {
    ...
    // New tests for minInv and maxInv validation
    @Test
    void testMinInvValidation() {
        partIn.setName("Test Part");
        partIn.setPrice(100.0);
        partIn.setInv(10);
        partIn.setMinInv(-1); // Invalid minInv
        ...
        assertEquals("Minimum inventory value must be positive", violation.getMessage());
    }

    @Test
    void testMaxInvValidation() {
        partIn.setName("Test Part");
        partIn.setPrice(100.0);
        partIn.setInv(10);
        partIn.setMinInv(0);
        partIn.setMaxInv(1001); // Invalid maxInv

        Set<ConstraintViolation<Part>> violations = validator.validate(partIn);
        assertEquals(1, violations.size());

        ConstraintViolation<Part> violation = violations.iterator().next();
        assertEquals("Maximum inventory value must be positive", violation.getMessage());
    }
}
